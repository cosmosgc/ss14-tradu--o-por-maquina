...
class LocalePath:
    def __init__(self, relative_file_path):
        self.pt = os.path.join(Project().pt_locale_dir_path, relative_file_path)
...
class LokaliseKey:
    ...
    def serialize_attr(self):
        return '.{name} = {value}'.format(name=self.get_key_last_name(self.key_name), value=self.get_translation('pt').data['translation'])

    def serialize_message(self):
        return '{name} = {value}'.format(name=self.get_key_last_name(self.key_name), value=self.get_translation('pt').data['translation'])

    def get_translation(self, language_iso='pt_BR'):
        logging.info(f'traduzindo "{self.data.key_name}"')
