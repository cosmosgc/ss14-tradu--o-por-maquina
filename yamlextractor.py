...
class YAMLExtractor:
    ...
    def execute(self):
        for yaml_file in yaml_files:
            ...
            pt_fluent_file_path = self.create_pt_fluent_file(en_fluent_file_path)
    ...
    def create_pt_fluent_file(self, en_analog_file_path):
        pt_file_full_path = en_analog_file_path.replace('en-US', 'pt-BR')

        if os.path.isfile(pt_file_full_path):
            return
        else:
            en_file = FluentFile(f'{en_analog_file_path}')
            file = FluentFile(f'{pt_file_full_path}')
            file.save_data(en_file.read_data())
            logging.info(f'Arquivo de localidade brasileiro {pt_file_full_path} criado')

        return pt_file_full_path
...
logging.info(f'Procure por arquivos yaml ...')
